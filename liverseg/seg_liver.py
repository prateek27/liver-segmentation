...
def preprocess_img(image, number_slices):
    ...
    """
#     print("Image Shape, number_slices = ", image, number_slices)
...
#                     print("-------", image[0])
                    img = np.load(image[0][i])
#                     print(img.shape)
                    images[j].append(img)
#                     break
#                     images[j].append(np.array(np.load(image[0][i])['section'], dtype=np.float32))
            else:
                img = image
    else:
        for j in range(np.array(image).shape[0]):
            for i in range(3):
                images[j].append(np.array(np.load(image[0][0])['section'], dtype=np.float32))
    ...
    with tf.Session(config=config) as sess:
        ...
        for frame in range(0, dataset.get_test_size()):
            ...
            if 1:
                ...
                #print(res_np)
                #print(np.max(res_np))
                #print(np.unique(res_np,return_counts=True))
                
                aux_var = curr_frames[number_of_slices//2][0]
                np.save(os.path.join(result_path, curr_ct_scan, aux_var)[:-4]+".npy", res_np)
                print('Saving ' + os.path.join(result_path, curr_ct_scan, aux_var)[:-4]+".npy")
                
                
                for i in range(number_of_slices):
                    aux_var = curr_frames[i][0]
                    if not os.path.exists(os.path.join(result_path, curr_ct_scan, aux_var)):
                        #res_np = res.astype(np.int32)[0, :, :, i]
                        np.save(os.path.join(result_path, curr_ct_scan, aux_var)[:-4]+".npy", res_np)
                        #print(res_np.shape)
                        print('Saving ' + os.path.join(result_path, curr_ct_scan, aux_var)[:-4]+".npy")
